...
// ... (Le début avec loadChartJs, constants etc. ne change pas) ...
    ...
    <section class="grid grid-cols-3 gap-4 text-center py-2">
      ...
      <div class="flex flex-col items-center justify-center relative group">
        ...
        <img src="${rankImg}" alt="rank" 
             class="rank-icon w-24 h-24 object-contain transform transition-transform duration-300" 
...
export async function mount(user) { /* ... (mount function remains exactly the same) ... */ try { await loadChartJs(); } catch (e) { return; } const scores = user.scores || {}; const muscuKeys = ["bench", "squat", "deadlift", "overhead_press", "vertical_row"]; const muscuData = muscuKeys.map(k => scores.muscu?.[k] || 0); const streetKeys = ["weighted_pullup", "weighted_dip", "front_lever", "full_planche", "human_flag"]; const streetData = streetKeys.map(k => scores.street?.[k] || 0); const cardioKeys = ["run", "bike", "rope"]; const cardioData = cardioKeys.map(k => scores.cardio?.[k] || 0); const createRadar = (canvasId, labels, data, labelName) => { const ctx = document.getElementById(canvasId); if (!ctx) return; new Chart(ctx, { type: 'radar', data: { labels: labels, datasets: makeDataset(labelName, data) }, options: commonChartOptions }); }; createRadar('chart-muscu', ["Bench", "Squat", "Deadlift", "OH Press", "Rowing"], muscuData, 'Musculation'); createRadar('chart-street', ["Tractions", "Dips", "Front Lever", "Planche", "Flag"], streetData, 'Street Workout'); createRadar('chart-cardio', ["Course", "Vélo", "Corde"], cardioData, 'Cardio'); }