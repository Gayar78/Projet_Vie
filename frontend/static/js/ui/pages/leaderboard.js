...
export const RANK_COLORS = {
  'iron': '#9ca3af',        // Gris
  'bronze': '#d97706',      // Bronze/Orange
  'silver': '#e5e7eb',      // Argent/Blanc
  'gold': '#fbbf24',        // Or
  'platinium': '#2dd4bf',   // Cyan
  'emeraude': '#10b981',    // Vert Émeraude
  'diamond': '#3b82f6',     // Bleu
  'master': '#a855f7',      // Violet
  'grandmaster': '#ef4444', // Rouge
  'challenger': '#facc15'   // Jaune Éclatant
};
...
export function rankFromPoints (score, max = 10_000) {
  ...
  return ['iron', 'bronze', 'silver', 'gold', 'platinium', 'emeraude', 'diamond', 'master', 'grandmaster', 'challenger'][idx]
}

export const categories = {
  general: { label: 'Général', sub: [{ value: 'general', label: 'Général' }] },
  muscu: { label: 'Musculation', sub: [{ value: 'total', label: 'Total' }, { value: 'bench', label: 'Développé couché' }, { value: 'squat', label: 'Squat' }, { value: 'deadlift', label: 'Soulevé de terre' }, { value: 'overhead_press', label: 'Développé militaire' }, { value: 'vertical_row', label: 'Tirage vertical' }] },
  street: { label: 'Street Work Out', sub: [{ value: 'total', label: 'Total' }, { value: 'weighted_pullup', label: 'Traction lestée' }, { value: 'weighted_dip', label: 'Dips lestés' }, { value: 'front_lever', label: 'Front lever' }, { value: 'full_planche', label: 'Full planche' }, { value: 'human_flag', label: 'Drapeau humain' }] },
  cardio: { label: 'Cardio', sub: [{ value: 'total', label: 'Total' }, { value: 'run', label: 'Course à pied' }, { value: 'bike', label: 'Vélo' }, { value: 'rope', label: 'Corde à sauter'}] }
}
...
<section class="page-enter relative min-h-[calc(100vh-160px)] px-6 pb-20 flex flex-col items-center text-white">
  <div class="mt-20 mb-12 text-center">
      <h1 class="text-5xl md:text-7xl font-black tracking-tight mb-4"><span class="bg-clip-text text-transparent bg-gradient-to-r from-white via-pink-200 to-pink-500 filter drop-shadow-lg">LEADERBOARD</span></h1>
      <p class="text-gray-400 text-lg max-w-xl mx-auto">Les légendes se forgent ici.</p>
  </div>
  <div class="flex flex-wrap justify-center gap-4 mb-10 relative z-20">
    ${['gender', 'cat','metric'].map(t=>`
      <div id="${t}-dd" class="relative group">
        ...
        <div id="${t}-menu" class="absolute left-1/2 -translate-x-1/2 mt-4 w-60 origin-top bg-[#0a0a0a]/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl shadow-black/50 opacity-0 scale-95 pointer-events-none transition-all duration-200 z-50 overflow-hidden"></div>
      </div>`).join('')}
  </div>
  <div class="w-full max-w-7xl grid lg:grid-cols-[1fr_320px] gap-8">
    <div class="liquid-glass-card rounded-2xl overflow-hidden flex flex-col h-[600px]" data-tilt data-tilt-glare data-tilt-max-glare="0.05" data-tilt-scale="1.00">
        <div class="grid grid-cols-[60px_1fr_80px_120px] gap-4 px-6 py-5 border-b border-white/5 bg-black/20 text-xs font-bold text-gray-500 uppercase tracking-widest">
            <div class="text-center">#</div><div>Athlète</div><div class="text-center">Rang</div><div class="text-right">Score</div>
        </div>
        <div id="lb-wrapper" class="overflow-y-auto scrollbar-hide flex-1 p-2">
            <table class="w-full border-collapse"><tbody id="leaderboard-body" class="text-sm"></tbody></table>
        </div>
    </div>
    <div class="flex flex-col gap-6">
      <div class="liquid-glass-card p-6 rounded-2xl space-y-4" data-tilt data-tilt-glare data-tilt-max-glare="0.05" data-tilt-scale="1.02">
        <h3 class="text-lg font-bold text-white flex items-center gap-2"><span class="text-pink-500">ℹ️</span> Info</h3>
...
export function initLeaderboardFilters () {
  ...
  const toggle = (btn,menu)=>{ if(menu.classList.contains('opacity-100')) hide(menu); else { show(menu); [genderMenu, catMenu, metMenu].forEach(m => { if (m !== menu) hide(m); }); } setTimeout(()=>{const away=e=>{if(!menu.contains(e.target)&& !btn.contains(e.target)){hide(menu); document.removeEventListener('click',away)}}; document.addEventListener('click',away)},0) }
...
export async function loadLeaderboard (gender = 'all', cat='general', metric='general') {
  ...
  // Skeletons
  tbody.innerHTML = Array(6).fill(0).map(() => `<tr class="border-b border-white/5"><td class="py-4 pl-4"><div class="skeleton w-8 h-8 rounded-lg mx-auto"></div></td><td class="py-3 flex items-center gap-4"><div class="skeleton w-10 h-10 rounded-full"></div><div class="flex flex-col gap-2"><div class="skeleton w-32 h-3 rounded"></div><div class="skeleton w-16 h-2 rounded"></div></div></td><td><div class="skeleton w-10 h-10 mx-auto rounded-full"></div></td><td class="pr-6"><div class="skeleton w-16 h-4 ml-auto rounded"></div></td></tr>`).join('');
  ...
  try {
    ...
    tbody.innerHTML = list.map((u,i)=> {
        const rankName = rankFromPoints(u.points);
        // Récupération de la couleur
        const rankColor = getRankColor(rankName);

        return `
      <tr data-id="${u.id}" class="group transition-all duration-200 hover:bg-white/5 ${u.id===myId ? 'bg-pink-500/10 border-l-2 border-pink-500' : 'border-b border-white/5 last:border-0'}">
        <td class="py-4 pl-4"><div class="w-8 h-8 mx-auto flex items-center justify-center rounded-lg font-bold text-sm ${i===0?'bg-gradient-to-br from-yellow-300 to-yellow-600 text-black shadow-lg': i===1?'bg-gradient-to-br from-gray-300 to-gray-500 text-black shadow-lg': i===2?'bg-gradient-to-br from-orange-300 to-orange-600 text-black shadow-lg':'text-gray-500 bg-white/5'}">${i+1}</div></td>
        <td class="py-3"><a href="/user/${u.id}" data-link class="flex items-center gap-4 group-hover:translate-x-1 transition-transform"><img src="${u.photoURL || '/static/images/ranks/rank.png'}" class="w-10 h-10 rounded-full object-cover ring-2 ${u.id===myId?'ring-pink-500':'ring-transparent group-hover:ring-white/20'} transition-all"/><div class="flex flex-col"><span class="font-semibold text-gray-200 group-hover:text-white truncate max-w-[140px] sm:max-w-[200px]">${u.displayName || 'Athlète'}</span>${u.id===myId ? `<span class="text-[10px] text-pink-400 font-bold uppercase tracking-wider">C'est vous</span>` : ''}</div></a></td>
        
        <td class="text-center">
          <!-- ICI : On applique la classe rank-icon et la variable CSS -->
          <img src="/static/images/ranks/${rankName}.png" 
               class="rank-icon w-10 h-10 object-contain mx-auto" 
               style="--rank-color: ${rankColor};"
               title="${rankName}">
        </td>

        <td class="pr-6 text-right"><span class="font-mono font-bold text-lg text-white tracking-tight">${u.points.toLocaleString()}</span><span class="text-xs text-gray-500 ml-1">pts</span></td>
      </tr>`}).join('')
  ...
  if (wrapper && myRow) setTimeout(() => myRow.scrollIntoView({ behavior: 'smooth', block: 'center' }), 100);
}

export function loadActivity () { document.getElementById('activity-feed').innerHTML = `<p>1. Sélectionnez une <span class="text-pink-400 font-bold">catégorie</span>.</p><p>2. Le classement est live.</p><p>3. Votre ligne est surlignée.</p>`; }